...
def get_my_tickets(customer_id):
    tickets = db.session.execute(
        select(Service_tickets).where(Service_tickets.customers_id == customer_id)
    ).scalars().all()
...
@token_required
def add_remove_mechanics(customer_id,ticket_id):
    try:
    ...
    query = select(Service_tickets).where(Service_tickets.id == ticket_id, 
                            Service_tickets.customers_id == customer_id)
...
@token_required
def add_remove_part(customer_id,ticket_id):
    try:
        Edit_Service_data = Edit_Service_tickets_Schema.load(request.json)
    except ValidationError as e:
        return jsonify(e.messages), 400
